...
* The instrument is blind to spellings it does not wrap. ``os.system``,
  ``os.popen``, ``os.spawn*``, ``ctypes`` process creation, a cached lookup
  performed at import time before the plugin loads, and a module that bound
  ``from shutil import which`` before the wrap are all invisible. A zero from a
  blind instrument is an absence of evidence.
...
CONTROL_MODULE_SOURCE = '''\
"""Planted positive control for tools/false_red_probe.py - ROADMAP OPS-74.
...
tree. Each test below is a deliberate specimen of one of the three outcomes
the probe claims to tell apart, so a run that fails to see all three has
proved its own instrument blind and must not report a zero.
"""
...
def planted_controls(parent: Path | None = None) -> Iterator[Path]:
    ...
    tmp.write_text(CONTROL_MODULE_SOURCE, encoding="ascii")
...
def probe(
    ...
    with planted_controls() as control_path:
