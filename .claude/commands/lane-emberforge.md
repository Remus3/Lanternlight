---
description: Emberforge math engine lane. Own the build and combat math. May not encode a number that has no measured source - omit the field instead, and keep unmeasured distinguishable from measured zero.
---
...
# Lane `emberforge` - Emberforge math engine

## Mandate

Own the build and combat math. May not encode a number that has no measured source - omit the field instead, and keep unmeasured distinguishable from measured zero.

## Your workspace

Your working directory is your own worktree, **`ll-lane-emberforge`**
under the worktree root, on branch **`lane/emberforge`**. The worktree
...
Commit and push to **`lane/emberforge`** freely. **Never merge to `main`**, never
