---
description: Capture and vision lane. Own passive screen capture and the frame-to-log wall-clock join that binds rendered text to numeric ids. Passive reading only - never synthesises input.
---
...
# Lane `capture` - Capture and vision

## Mandate

Own passive screen capture and the frame-to-log wall-clock join that binds rendered text to numeric ids. Passive reading only - never synthesises input.

## Your workspace

Your working directory is your own worktree, **`ll-lane-capture`**
under the worktree root, on branch **`lane/capture`**. The worktree
...
Commit and push to **`lane/capture`** freely. **Never merge to `main`**, never
