---
description: Data ingest lane. Own every reader of a surface the game writes - the log parser and tail, the GVAS save reader, the market cache, and path resolution. Never emits a value it did not measure.
---
...
# Lane `ingest` - Data ingest

## Mandate

Own every reader of a surface the game writes - the log parser and tail, the GVAS save reader, the market cache, and path resolution. Never emits a value it did not measure.

## Your workspace

Your working directory is your own worktree, **`ll-lane-ingest`**
under the worktree root, on branch **`lane/ingest`**. The worktree
...
Commit and push to **`lane/ingest`** freely. **Never merge to `main`**, never
