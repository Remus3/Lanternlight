...
| Session watcher | **Entry point done, arming is MANUAL** | `lanternlight.armwatch` arms all four capture surfaces from one command: `python -m lanternlight.armwatch --dest-root C:/ll-captures/<today>`. **It has to be invoked** - two sessions running launched the client with nothing armed. `--dest-root` is required and carries no date logic. Automating the arming is `ROADMAP.md` item 4d |
