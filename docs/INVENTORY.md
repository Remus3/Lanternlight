...
| `tests/test_no_environ_mapping_assertions.py` | An assertion anywhere in this repository putting the ENVIRONMENT MAPPING in an operand position. `assert "NAME" not in os.environ` names one variable and looks like the safest line in the file; pytest's assertion rewriting prints the whole right-hand operand, so on failure it renders EVERY variable and EVERY value the process holds. Measured here 2026-09-20 with a planted sentinel and a planted admin-family key shape - the sentinel appeared three times in the failure output and the planted key was visible in the `+ where` line - while the control `assert os.environ.get("NAME") == "x"` rendered only the retrieved value. The rule is therefore about the MAPPING OBJECT reaching an operand, not about whether the author mentioned the environment: inside an `assert`, `os.environ` may only be subscripted or be the receiver of `.get(...)`. It parses rather than greps, because a line-oriented pattern is a claim about line breaks and this repository has published a wrong answer from one. Six positive specimens and four negative ones drive the real detector, and the tree arm carries a scanned-file floor so a walker that stops yielding reds instead of passing vacuously. Filed from Clockspeed's 2026-09-20 ACTION note; the one site it found was in `tests/test_gitignore_shadowing.py`. |
