...
| `tests/test_history_scan.py` | `scripts/history_scan.py` missing a value that lives only in HISTORY (`OPS-106`). The load-bearing arm plants a synthetic identifier in a throwaway repository, commits it, DELETES the file and requires the scan to find it and name the introducing commit; a clean repository is the control and must report zero. Further arms cover a credential class, an encoded identifier, the git-identity value half and the non-ASCII rule with its binary-suffix exemption, and assert the report never carries the planted value or any fragment of it. Five arms were added after an adversarial mutation pass found five survivors: a value reachable only from a side branch or a tag, two distinct identifier labels, a path that is itself an identifier (withheld), an identity built by string concatenation (the joined pass), and a blob re-added later (first-introducing commit must stay the earliest). Re-measured with that harness: all 9 mutants killed - walking HEAD instead of --all, dropping the encoded, credential, non-ASCII, path-withholding, joined or value-half pass, last-wins first-commit tracking, and narrowing the plain labels. |
...
| `tests/test_scratch_path_guard.py` | A tracked command building a scratch path that lands in the Git for Windows install root (`OPS-107`): an unset-prone temp variable in a shell context, the bare (null) temp variable and the unset environment `TMPDIR` in PowerShell, the same `TMPDIR` in cmd and in Python environment reads, a root path that only BEGINS with the temp directory's name (which Git Bash resolves into the install root, unlike the mounted temp directory itself), and a root temp path in a Python string (which native code resolves on the drive root), refused by `ops/scratch_path_guard.py`. Scope is a CONTEXT, not a file list - every line of scripts, hooks, config and agent-instruction text (`.claude/**`, the hand-off, `CLAUDE.md`, `docs/HEADLESS.md`), every fence and indented block in other Markdown so prose may describe the bug, and non-docstring string constants in Python. Every specimen is assembled at run time; the whole tracked tree must scan to ZERO, which is the measured false-positive rate; and every generated lane contract must name the session scratchpad as the only scratch destination. Run by `python -m ops.preflight`. |
...
| `scripts/history_scan.py` | The one-off full-HISTORY scan (`OPS-106`). Every tree guard scans the tree, and a value committed and later deleted is still in every clone. Walks every blob reachable from every ref with one `git cat-file --batch`, and runs `lanternlight/redact.py`'s plain, encoded and git-identity detectors, `tools/secret_scan.py`'s credential classes and the 7-bit ASCII rule with `tests/_tracked.py`'s binary-suffix exemption - all reused, none copied. Prints class, count, commit-relative path and first-introducing commit ONLY, never a value, and withholds a path that is itself a hit. Exit 0 clean, 1 findings, 2 did not run. It never rewrites anything: rewriting a public history is a ledger question, not a session decision. |
