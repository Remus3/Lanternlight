...
| `tests/test_hook_command_roots.py` | A hook command in `.claude/settings.json` naming an absolute repository root (`OPS-61`). Every hook script resolves its repo root from its own file location, so an absolute command makes a clone's or a worktree's hook FIRE, SUCCEED and answer about the original tree - measured, it also WROTE that tree's records. The guard is `tools/hook_command_guard.py` and it reports three spellings: drive root, UNC root and POSIX root. |
