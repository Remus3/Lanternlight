...
The OLDEST entries of [`docs/LEDGER.md`](LEDGER.md), moved out whole so
...
Entries in this archive: 200.
Newest archived: LL-0200 - 2026-09-08 - The four missing GitHub community-standards documents were written for this project rather than templated, the README was re-measured from a fresh clone, and repo topics went 15 to 19
...
**Evidence:**
- Suite `1772 passed in 188.39s`, run BARE and read off the summary line. Ruff `check` clean. Merge gate OK at 1772 against the **1760** baseline standing at the previous commit. Client **closed**. Watcher `ARMED` / identity confirmed, pid 21452.
- **PROVOKED FIRST, which is what the item demanded, and the provocation SPLIT it.** Four scenarios, each a real `run_rolling` against a real temp `Saved` tree, with `check_watcher` given path overrides so nothing read or wrote the live record. A destination refusing the COPY - once by a genuine filesystem `PermissionError` with no mocks at all, once by `OSError(28)`, the `ENOSPC` this machine hit - completed 12 passes, advanced the heartbeat, held all four surfaces inside their thresholds, archived **ZERO** files and read `ARMED` / `VERIFIED`. **CONFIRMED.**
- **The same provocation REFUTED the item's headline.** A destination refusing the MKDIR is already caught: `mkdir` sat outside `_copy_one`'s `try`, so it raised through `poll_once` and killed that surface's thread, and `4f`'s machinery NAMED the surface - `ARMED` at 5 s and 40 s, `SURFACE_STALE` naming `savegames` at 75 s and 85 s against its 69 s window. So "cannot WRITE reads ARMED forever" is true of the copy and FALSE of the mkdir. The heading was left as written and the refutation put beside it.
- **THE FIX, and the AFTER reading is the acceptance.** `SaveWatcher.consecutive_failed_passes` counts passes that offered files and archived none; `run_rolling.record_pass` stops recording a surface at 3 such passes, so it falls out of its freshness window and the EXISTING reader names it - criterion 2 forbade inventing a channel `check_watcher` does not read. Measured on the fixed code, threaded, every copy refused, reproduced twice: `ARMED` at 5 s and 40 s, **`SURFACE_STALE` naming `savegames` at 78 s and 88 s**, zero archived throughout.
- **Criterion 3 is N/A and that is a result, not a skip.** The item demanded a BOUNDED error record so a crash-looping watcher could not worsen the disk `OPS-14` is about. The fix writes NOTHING - freezing emits no file, and the two `say()` lines fire once per TRANSITION into a stream `default_spawn` sends to the null device. No log, so no bound.
- **Criterion 4 - eight mutations, eight RED, both modules restored byte-exact:** `record_pass` always records (2 failed/125), threshold 3->1 (1/126), revert the THREADED call site (2/125), revert the synchronous one (3/124), reset-on-idle reverted (1/126), failures never counted (7/120), `mkdir` moved back above the `try` (1/126), `replace` moved back below the `except` (1/126). Every anchor asserted `count == 1` on the way in AND on the way back.
- `poll_once`'s docstring promised "never raises" and it was FALSE. Both raising calls - `dest_dir.mkdir` and `tmp_target.replace` - are now inside the `try`.

**THE REFUTATION REFUSED THE FIRST CUT AND WAS RIGHT FOUR TIMES. Fifth cycle running that it caught what the suite could not.** Every measurement in the first cut survived it; four things the first cut ASSERTED did not.
**THE PRODUCTION PATH WAS DECORATION - the worst of the four.** `run_rolling` records a pass in TWO places: a synchronous branch driven by `max_passes`, and `poll_forever`, the threaded loop `default_spawn` actually runs. All nine new tests drove the synchronous branch, because a surface's poll interval is fixed by `session_plan` and is not injectable. **Reverting the THREADED call site alone left the ENTIRE suite at `1769 passed`** - the guard existed and production did not have it. This repo has hit the identical two-call-site defect once already, on the `4e` heartbeat. Now guarded by `TestBOTHCallSitesRouteThroughTheFreeze`, which walks the AST and asserts every `heartbeat.record` sits inside `record_pass` - the technique `tests/test_process_capability.py` already uses. **It is a deliberate second-best and says so: it proves the two call sites cannot DIVERGE, not that the threaded loop behaves.**
**A DELIBERATE DESIGN DECISION WAS MEASURED AND WAS WRONG.** The first cut left the count alone on an idle pass, with a comment arguing that a destination breaking while nothing changed should still accumulate evidence. Measured: three transient failures, then the source goes quiet - the transient save's actual life cycle - and the count stays pinned at 3 through recovery and **200 idle passes**. On sources this same session measured quiescent for FIVE DAYS, a recovered destination would never be forgiven. An idle pass now resets. **A comment that argues for a choice is not evidence that the choice is right.**
**A HALF-FIX WAS DECLARED A FIX.** Moving `mkdir` inside the `try` was called a fix for `poll_once` "never raises" while `tmp_target.replace` sat outside it - and an ordinary Windows read-only attribute, no mocks and no exotic ACL, raises `PermissionError [WinError 5]`, kills the thread and leaks the `.part`. **Moving one of two raising calls and declaring the promise kept is a shape this repo keeps paying for.**
**THREE FALSE SENTENCES WRITTEN THE SAME HOUR AS THE CODE - `OPS-16`'s lesson, third recurrence.** (1) "covered by 24 assertions" - 24 is the token's OCCURRENCE count and the assert lines are 13: a filed count, in a docstring, in the cycle that had just paid for one; the number is now gone rather than corrected. (2) The `STATED COST` arithmetic was wrong twice - the first pass is at t=0 so the third falls at `2 x poll`, making `logs` freeze at 10 minutes not 15, and the staleness window runs from the last RECORDED stamp, so it is named at 21 minutes not 31. (3) The constant cited `SURFACE_STALE` at 75 s and 85 s as evidence the mechanism works - those readings are from the PRE-FIX provocation where a thread had DIED, a different branch of the reader entirely.
**A MIS-ATTRIBUTION KEPT RATHER THAN FIXED, because it cannot be fixed.** The count cannot tell a refusing DESTINATION from an unreadable SOURCE. The first cut's message said "has been refused", which reports a locked source file as a destination fault. It now states only what was observed - files were offered and none were archived - and attributes no cause.
**`write_text` TURNED THREE FILES CRLF MID-CYCLE**, exactly as `CLAUDE.md` warns, and the tell was a byte-anchored mutation refusing to match. `.gitattributes` would have normalised the blob so the commit was never at risk, but every LF-anchored scripted edit silently stops matching. Converted back with `write_bytes` and re-verified at 0 CRLF.
**LIMITS, stated rather than implied.** The threshold is justified by measurement for `logs` alone - the ten snapshots proving the game does not hold its log exclusively are all `logs`, and for a 3 s surface three passes is six seconds. What makes that tolerable is the WINDOW, not the count: a frozen surface is only REPORTED once its own staleness threshold has elapsed too, measured at 78 s against 69 s. If a save file is ever measured locked longer than that, the number needs revisiting. And no behavioural test drives the threaded loop; the end-to-end evidence for it is the provocation, run by hand.
**ITEMS 7, 10, 11 AND 12 ARE NOT CREDITED.** The client was closed all session. Nothing was done to the game or its directory: no game process was touched, and every probe of `C:/ll-captures` and the `Saved` tree was strictly read-only.

### LL-0134 - 2026-09-05 - OPS-14's growth join ANSWERED - all 40 files reconcile - and the refutation caught the new item citing tests that did not cover it

**Evidence:**
- Suite `1760 passed in 111.56s`, run BARE at the wrap and read off the summary line. Ruff `check` clean. Merge gate OK at 1760 against a **1757** baseline measured with `--collect-only` BEFORE any slice was dispatched; the refutation re-derived that baseline independently by extracting HEAD with `git archive` and collecting there, and also got 1757. Client **closed**. Watcher `ARMED` / identity confirmed, pid 21452.
- **`OPS-14`'s open join is ANSWERED, and it needed no correlation** - the two sides reconcile exactly. The client has not launched since local `2026-08-30 21:11:46`, the newest mtime anywhere under the game's `Saved` tree across all 17 files, with zero modified since 2026-08-31. Derived a SECOND way from log CONTENT rather than the filesystem: four recoverable sessions, three play and one 24-second launch that never selected a class, local 2026-08-25 18:34:46 to 2026-08-30 21:11:43, with a rotation-chain proof closing the gaps - every launch rotates the log exactly once and no backup stamp in live or archive is unexplained. Sessions BEFORE 2026-08-25 are UNMEASURED and explicitly not zero.
- **Four watcher armings inside that dead window archived BYTE-IDENTICAL sets** - stamps `20260831-170906`, `20260901-075014`, `20260901-202636` and `20260903-185354`, each 13 files and 13,019,536 bytes. Verified two independent ways: a digest over the sorted per-file sha256 list, and the refutation's `filecmp.cmp(shallow=False)` across all 6 pairs x 13 files = 78 comparisons, zero mismatches. `SaveWatcher._seen` is in-memory, so every restart re-snapshots the whole source set.
- **This item's own 'up 40 files / 0.04 GB in five days' now reconciles FILE BY FILE with nothing left over:** 19,162 + 13 + 13 + 13 duplicate snapshots + 1 analysis CSV = **19,202 files, 10,639,269,122 bytes, 9.9086 GiB**, measured this run by a full walk. Bytes: 3 x 13,019,536 + 33,233 = 39,091,841. **Not one byte of the five-day growth is new game data.**
- **The dating INSTRUMENT, measured tree-wide over all 431 watcher snapshots:** `shutil.copy2` carries the source's mtime onto the copy, so an archived file wears the game file's modification time. Worst single misdating **25.44 days**, and `find -newermt 2026-08-31` over the whole tree returns three files and NONE of the thirteen archived on 2026-09-03. Creation time DOES record the archive moment and agrees with the filename stamp on **431 of 431** within 5 s, but is not durable - a `copy2` of a snapshot kept its 2026-08-09 mtime and took a fresh 2026-09-05 creation time. **mtime agrees with the stamp on 260 of 431, 60.3 percent**, because the live log and the transient save are rewritten moments before archiving, so spot-checking CONFIRMS the wrong instrument.
- `tests/test_savewatch.py::TestTheArchiveMomentIsNotTheMtime` pins both clocks. Watched going red: mutating `shutil.copy2` to `shutil.copy` gave **3 failed, 25 passed**, and the module was restored byte-identical, sha256 `23af4792459d2571db73922a66f53fabd52e533d576974ed58ea0109903cfd7d`. **One of those three failures was pre-existing and does NOT count as cover** - it monkeypatches `copy2` and reddens on its spy going uncalled, not on the clock, so the swap is loud in a way that names the wrong cause.
- `ROADMAP.md` `OPS-26` filed: a watcher whose destination stops accepting writes reads `ARMED` forever and leaves no trace. Four steps read from source - `_copy_one` swallows `OSError`; `poll_once` records `_seen` only on success; `poll_forever` calls `heartbeat.record` UNCONDITIONALLY after the pass; `default_spawn` passes `stdout=DEVNULL, stderr=DEVNULL`. It also answers the MECHANISM half of `LL-0124`'s 'nothing in this repo records a watcher's exit'.

**THE REFUTATION REFUSED THE MERGE ON PROSE, AND WAS RIGHT THREE TIMES.** Every measurement and every line of code survived it - the byte-identity, the 431, the 60.3 percent, the 25.44 days, the mutation counts and the suite figures all confirmed exactly, with 431 re-derived through three progressively looser regexes that each returned 431 with zero near-misses. What did not survive were three sentences the merger wrote ABOUT that evidence. **Fourth cycle running that the refutation caught what the suite could not.**
**AN ITEM CITING A TEST AS COVER IS A FILED COUNT WEARING BETTER CLOTHES.** `OPS-26` claimed 'steps 1 and 2 have tests behind them'. Step 2 is the retry-forever half, and **every failure test in that file calls `poll_once` exactly ONCE** - they prove the failure is TOLERATED and say nothing about the retry, which `_copy_one`'s own docstring promises in prose. The item drew an explicit tested-versus-reasoned line and then put a step on the wrong side of its own line. Fixed by ADDING the test rather than weakening the claim: `test_a_failed_copy_is_RETRIED_on_the_next_pass`, watched going red - moving `_seen.add` out of the success branch gives **1 failed, 28 passed**, and that mutation was invisible to all 28 tests before it.
**AN EMPTY GREP WITH A PASSING POSITIVE CONTROL IS STILL A CLAIM ABOUT THE PATTERN.** The merger swept every tracked `.md` and `.py` whitespace-collapsed and case-insensitively - the method this repo mandates - and concluded the mtime-dating trap was recorded NOWHERE. It is recorded in `LL-0104`, which dated these very snapshots by 'NTFS CreationTime 07:50:14 on all 13' and used the same move to correct an earlier entry. The patterns asked about 'mtime' and 'archive time'; the prior art says 'CreationTime' and 'backup moment'. **The sweep's own positive control PASSED**, because the newly written file matched it - so the control proved the instrument RAN, not that it asked the right question. Search for the MECHANISM under every name it might wear.
**A COUNT OF ONE WAS WRONG.** `OPS-26` said one launch in `C:/ll-captures` left watcher output. Three did - `2026-08-25b`, `2026-08-30` and `2026-08-31` - found by sweeping for the filename instead of reading the one directory the merger happened to have open. An enumerated inventory is a filed count.
**WHAT THIS DELIBERATELY DOES NOT CLOSE, written down rather than implied.** 'A watcher that had silently stopped would produce the same flat line' is still TRUE and cannot be separated while the sources are quiescent, because a correct watcher and a dead one both write nothing. Zero archives is CONSISTENT with health, never evidence of it. The 2026-09-03 arming landing all 13 of its files proves the copy path worked in THAT process at THAT moment and nothing after it. `OPS-14`'s headline - what consumed tens of gigabytes - is untouched and still needs the operator-scale scan.
**ITEMS 7, 10, 11 AND 12 ARE NOT CREDITED.** The client was closed all session and nothing was measured for any of them. Nothing was done to the game or its directory: no game process was touched, the watcher copies out and never writes to the source, and every probe of `C:/ll-captures` and the `Saved` tree was strictly read-only.
...
**Evidence:**
- `ops/loop/guard.py`: `_windows_pid_is_alive` now reads the EXIT time out of `GetProcessTimes` instead of comparing `GetExitCodeProcess` against `STILL_ACTIVE` (259). Zero until the process exits, a timestamp afterwards, a different field from the exit code, and answerable under the right the module already held.
- Full suite `1647 passed in 105.64s`, run BARE and read off the summary line. Merge gate OK at 1647 collected against a 1637 baseline measured with `--collect-only` BEFORE any slice was dispatched. Ruff clean.
- RED observed against HEAD by THREE independent parties - the implementing slice, the blast-radius sweep and the merger - at `tests/test_loop_guard.py::test_a_reaped_child_reads_dead_whatever_exit_code_it_chose[259]`. The five neighbouring exit codes 0, 1, 42, 258 and 260 all read correctly dead before and after, which is what separates a fixed collision from a special-cased number.
- THE SAFETY GUARD CHANGED AND WAS RE-PROVED BY MUTATION, not assumed. Dropping `GetExitCodeProcess` made `guard`'s win32 inventory EQUAL to `watch`'s, retiring a discriminator in `test_the_guard_module_and_the_watch_module_differ_where_expected`. The assertion was rewritten rather than deleted, and `_scope_source` forced to read `guard.py` for every path still reddens that test - the exact double-read failure it exists to catch.
- NO allowlist entry was needed: every call the new probe makes was already in `ALLOWED_WIN32_FUNCTIONS`, and no right beyond `PROCESS_QUERY_LIMITED_INFORMATION` is requested.

**TWO DESIGNS WERE BUILT AND A DISTINCT AGENT CHOSE BETWEEN THEM, which is the only reason the right one shipped.** The implementing slice built `WaitForSingleObject(handle, 0)` - documented semantics, and its defence of `SYNCHRONIZE` as purely observational was CORRECT. It lost on REACH. With `SeDebugPrivilege` dropped from the probing token, 77 of the 312 pids this machine can open with `PROCESS_QUERY_LIMITED_INFORMATION` DENY `PQLI | SYNCHRONIZE` with `ERROR_ACCESS_DENIED` - 24.7 percent, all service processes. Every one lands on that design's fallback, which is the original buggy comparison verbatim; driven through the shipped function with `SYNCHRONIZE` denied it returned True for a child that exited 259. It would have shipped the item unfixed on a quarter of the pids it can be asked about, in a branch its own diff never exercised.

**AND ITS CORRECTNESS WOULD HAVE BEEN A FUNCTION OF THE LAUNCHING TOKEN**, which is exactly the fresh-clone case this repo writes its rules for. A loop started from an ordinary shell or a scheduled task sees the 24.7 percent; a session like this one sees zero.

**THE FIRST SWEEP OF THAT POPULATION READ ZERO AND IT WAS AN ARTIFACT OF THE INSTRUMENT.** This session runs with `SeDebugPrivilege` ENABLED, which bypasses the DACL check in `OpenProcess`, so the residual set could not have been non-empty whatever the security descriptors said. Second cycle running that a reading turned out to be a claim about the instrument - last cycle it was a held handle, this cycle a privilege set.

**THE COST ON THE CHOSEN SIDE IS WRITTEN INTO THE DOCSTRING, not just known.** Microsoft documents `lpExitTime` as undefined while the process has not exited. The zero is MEASURED, not promised - 0 of 312 running processes reported a non-zero exit `FILETIME`. A guard that mis-states its coverage is worse than one with a declared hole, which is `OPS-16`'s lesson applied to the fix that replaced it.

**A REJECTED THIRD DESIGN, recorded so it is not re-proposed:** reading the exit time only when the exit code is 259, as belt and braces. It buys nothing - a live process ALWAYS reports 259, so the undefined field is read in exactly the same cases either way. The merger proposed it and the adjudicator had already refuted it.

**TWO OF THIS ITEM'S OWN SENTENCES WERE WRONG AND ARE WITHDRAWN IN `ROADMAP.md` ADJACENT TO THEM, both written by the merger the same day.** (1) 'Believed alive forever' overstates: the false ALIVE needs a live handle to the exited process object, and the shipped `default_spawn` drops its `Popen` at `return child.pid`, so nothing in the detached topology holds one. Nothing in this repo exits 259 either - `armwatch.main()` returns 0 or 2, and `taskkill /F`, `TerminateProcess`, an uncaught exception and an argparse error give 1, 1, 1 and 2. The defect was real and summonable but NOT firing. Fixed anyway, on the `LL-0124` principle that a disarmed check fires the cycle after it ships. (2) The consumer count said six; it is FIVE - two of the cited lines are not call sites at all but f-strings recording the ANSWER, and `armed_pid` was missing. Third filed count wrong in two cycles, and the CORRECTION was itself loose on its first pass - it called those two lines branches of one call - which the refutation caught.

**THE WORST CONSUMER WAS `check_watcher`, and it is worse than the item said.** Its `DEAD` branch is skipped and every reachable verdict falls OUTSIDE `REARM_STATES`, so the wrap never re-arms. `IMPOSTOR` is UNREACHABLE, because `process_creation_time` still succeeds on the exited process - a held handle serves `GetProcessTimes` as happily as `GetExitCodeProcess` - so `_identity_matches` returns True. Both the `4e` identity check and the `4f` surface check are defeated by it.

**FOUR NEW ITEMS CAME OUT OF THIS ONE**, none folded into it: `OPS-19` (`pid_is_alive` calls 12 RUNNING processes DEAD on access-denied - a fail-OPEN in a function documented as fail-closed, found independently by three passes and surviving this fix untouched), `OPS-20` (nothing tests `guard.py`'s access mask while a docstring says it is covered - the `OPS-16` sin again), `OPS-21` (`read_owner` folds four facts onto `None`, so a corrupt lock file is reclaimed and overwritten), and `OPS-22` (`precommit_gate` matches a forbidden cmdlet as a bare substring, so a MENTION is blocked like a CALL - it fired on the sweep that found it, and it is the same sentinel-is-also-a-legal-datum shape as `OPS-18` itself).

**THE REFUTATION REFUSED THE MERGE AND WAS RIGHT TO. Third cycle running that it caught what the suite could not.** It found THREE FALSE SENTENCES in the shipped `guard.py` docstring - the `OPS-16` failure mode reproduced verbatim in the fix that replaced it. (1) The docstring claimed only two things return False and that every undecided case returns True; measured with `SeDebugPrivilege` dropped, 13 access-denied pids were STILL RUNNING half a second later and all 13 read dead - so the docstring asserted the exact opposite of `OPS-19`, filed in the same commit. (2) It claimed a child that exited 259 reported a ZERO exit timestamp; it reports a non-zero one, and if it reported zero the fix would not work at all. (3) It quoted 77 of 312 denying `SYNCHRONIZE` WITHOUT the `SeDebugPrivilege`-dropped condition that makes the number true - 0 of 338 deny under this session's token. A caveat stated in the roadmap but dropped from the artifact is a lie in the artifact. All three corrected before the commit.

**AND THE FAIL-CLOSED PROMISE WAS GUARDED BY NOTHING.** The refutation flipped the `GetProcessTimes`-failure branch to fail OPEN and the suite stayed green at 221 passed, rc=0. Three injected tests now pin it, and the same mutation is caught: `1 failed, 33 passed`. A green suite says nothing about a branch no test reaches - and the branch nobody tested was the one carrying the promise the whole module rests on.

**THE PII BACKSTOP FIRED ON THE FIX'S OWN FIXTURE, correctly.** A realistic 18-digit `FILETIME` used as test data matched `test_no_pii.py`'s long-identifier rule. The value is arbitrary - the probe compares against zero and nothing else - so the constant changed and the rule did not. Narrowing a redaction guard to make a test pass is the one move that is never available.
...
**Evidence:**
- Suite 1634 passed in 114.26s, run BARE at the wrap; baseline 1594 before the fix and 1560 before the item. ruff check . clean. merge_gate.verify(baseline=1594) OK at 1634.
- NEW tests/test_process_capability.py, 74 tests. It builds a symbol table of what each bound NAME refers to and routes every access through the same capability checks whether it is spelled as an attribute, a literal getattr, or a bare name introduced by a from-import. Checks: library allowlist (kernel32 only), Win32 function allowlist, os attribute allowlist, os.kill signal must be literal 0, dynamic access, subprocess argv must be a list literal starting with sys.executable, the executable= kwarg refused, and an import allowlist.
- PREMISE MEASURED BY THE MERGER BEFORE BUILDING, with controls: replaying the OLD guard's exact logic, all three OPS-16 spellings slipped past (callees seen were run/str, Popen/str, getattr, and NtSuspendProcess/WinDLL - none in the 7-name forbidden set) while kernel32.TerminateProcess and os.kill(pid,9) were caught. The premise was true.
- VERIFIED BY THE MERGER AFTER THE FIX, each with realistic context and three controls that must still pass: from os import system; executable= kwarg; getattr(os,'system'); getattr(os,'kill')(pid,9); getattr(subprocess,'run'); alias = kernel32 then alias.TerminateProcess; from ctypes import WinDLL as W; getattr(ctypes.windll,'ntdll'); vars(os)['system'] - ALL REFUSED. Controls getattr(plan,'name',None), Popen([sys.executable,...]) and os.kill(pid,0) still ALLOWED. Both real modules scan to 0 violations.
- os.kill is permitted ONLY with a literal 0. That nuance is the whole design: ops/loop/guard.py's POSIX existence probe is os.kill(pid, 0) and is safe precisely because of its argument, while the old denylist banned the NAME at any signal. The danger is the capability, not the spelling.
- 33 mutations run, 33 killed, 0 survivors, every anchor asserted to match exactly once. Two survived the first pass - both real untested branches - and got tests.
- The mirror test asserting both real modules produce zero violations is present, and watch.py's scan reports dyn: ['getattr'], proving its two legitimate getattr(plan, ...) calls were SEEN and passed rather than missed.
- No production code changed - git diff on ops/loop/watch.py and ops/loop/guard.py is empty. The anti-cheat boundary was re-inventoried and is clean: the new file imports ast, dataclasses, pathlib and pytest and executes no ctypes or subprocess.

THE FIRST IMPLEMENTATION WAS REFUTED AND WAS NOT CLOSE-ABLE. It shipped with 11 undeclared holes - from os import system; the executable= kwarg, which makes an argv allowlist worthless because a caller can name a different program in a keyword; the whole getattr laundering family (os, subprocess, ctypes.windll); and handle rebinding through an alias, with, for and tuple unpacking.
WORSE THAN THE HOLES: the docstring ASSERTED coverage it did not have. It claimed a literal-name getattr 'cannot launder an entry point the attribute form would have failed on' and named os.system, os.killpg and os.abort as caught. Both false, each falsified in one command. OPS-16's own acceptance is that the blindness must be stated in the artifact, so a guard that MIS-states its coverage fails the item outright rather than partially.
OPS-16'S OWN LIST OF THREE WAS INCOMPLETE. Building the allowlist surfaced os.system and os.killpg as a fourth and fifth spelling, reachable through the legitimate os import, and os.abort as a sixth. An enumerated list of blind spots is itself a filed count.
THE MERGER'S BRIEF WAS WRONG TWICE, and both were caught downstream. (1) I handed the first agent an inventory of both modules as authoritative and told it to ban getattr outright while asserting both modules would pass - incompatible, because watch.py:1038-1039 already calls getattr(plan, 'name', None). Hand an agent the METHOD to re-derive an inventory, not the inventory. (2) I then handed the FIX agent a table of 21 holes, and two of them were not holes - self.k32 = WinDLL(...) was already flagged, though with a wrong message naming k32 rather than TerminateProcess, and import ctypes as c already passed. The agent said so rather than accepting the table, which is the same over-claim this item exists to end.
MY OWN PROBE WAS THE BROKEN INSTRUMENT, TWICE THIS SESSION. Running scan_source over bare one-line snippets with no imports and no library binding produced FOUR false 'allowed' readings and I was about to report the agent's claims as false. With realistic context every one was refused. Earlier the same shape: an acceptance probe evaluated a real Heartbeat against a fabricated clock a day ahead and read STALE everywhere. An empty or passing result is a claim about the INSTRUMENT - including the instrument the merger writes to check an agent.
THE MERGE GATE CAUGHT A RED NEITHER AGENT COULD SEE. Both slices reported their own files green; the gate reported 2 failed, 1592 passed. A NEW tracked file must be given a lane owner in ops/lanes.py or nothing arbitrates a concurrent edit to it. Assigned to the SAFETY lane, whose mandate is every repository hygiene guard and never weakening a guard to make a build pass. Then scripts/write_lane_contracts.py, because the roster is not the only copy of itself. This is a trade-off of the orchestration, not an agent failure: slices are told to run only their own test file so a concurrent half-written file does not read as their own failure, which necessarily blinds them to integration failures. The gate is the compensating control and it fired on the first run.
FILED, NOT FIXED: OPS-17. The refutation's first full run also hit an intermittent red unrelated to this item - test_process_creation_time_is_none_rather_than_a_guess_when_it_cannot_tell, with pid 16264 REUSED between being reaped and being probed. _dead_pid() exists to avoid guessing a pid and its docstring reasons about reuse, but its `with subprocess.Popen(...)` block closes the process handle on exit, which on Windows is exactly when the pid becomes eligible for reuse. The helper reopens the hole it was written to close.
...
**Evidence:**
- Suite 1560 passed in 108.13s, run BARE and read at the wrap; baseline 1547 measured the same way before the fixes, and 1518 before the item began. ruff check . clean. merge_gate.verify(baseline=1547) OK.
- ops/loop/watch.py: seventh state SURFACE_STALE - the combined stamp is inside its threshold but at least one INDIVIDUAL surface has stopped advancing against its own poll interval, or never recorded at all. The status NAMES which. REARM_STATES is still exactly NO_RECORD, DEAD, IMPOSTOR, and nothing is terminated for any state.
- lanternlight/armwatch.py: the heartbeat is SELF-DESCRIBING - a new top-level intervals map beside an unchanged surfaces map - so the reader never re-types a cadence the watcher owns. Both production call sites pass it (armwatch.py:744 and :766); a mutation dropping the argument at the THREADED site was caught by exactly one test, and dropping it at the bounded site left that test green, which is what proves the two sites independently covered.
- Per-surface threshold SURFACE_STALE_MULTIPLE * poll + 2 * flush, the same k=3 as the combined one and BOUND to it rather than re-typed, giving 69/69/150/960 s. No poll interval or flush value is re-typed as a literal in the ops layer.
- MEASURED END TO END BY THE MERGER with a REAL Heartbeat writing a REAL file, not a hand-built payload: all four surfaces fresh reads ARMED; logs never recorded reads ARMED inside the grace window at 100 s; and SURFACE_STALE naming ('logs',) at 2000 s and at 100000 s. The payload in those runs has intervals and surfaces BOTH missing logs, which is the writer-emittable shape the pre-fix tests could not produce.
- A real threaded run_rolling for 330 s, SAMPLED every 10 s, read ARMED 33 of 33 with worst observed ages savegames 33.0 s, savedroot 30.0 s, logs 290.1 s against thresholds of 69, 150 and 960. A healthy watcher is not reported wedged.
- Non-vacuousness: Slice A ran six mutations, Slice B four, the two fix agents six between them, and the refutation pass eight - every one with its anchor asserted to match exactly once, watched RED, and restored.

THREE DEFECTS WERE FOUND BY THE REFUTATION PASS AFTER BOTH SLICES REPORTED DONE, and two of them meant 4f's own acceptance was not met in production. All three were invisible to a green 1547-test suite and to the merge gate. This is the second cycle running in which the refutation, not the suite, was the thing that caught the real defect.
(1) IT CRIED WOLF. Heartbeat._flush_locked set _last_flush BEFORE attempting the write and swallowed OSError, so a FAILED flush still burned the full 30 s throttle window. Reproduced on a healthy watcher polling exactly on cadence: t=70, failed_writes=2, SURFACE_STALE stale=('savegames',). Two failed flushes ate 60 s of a 69 s budget. Fixed: _last_flush now records the last SUCCESSFUL write. A check that cries wolf on a healthy watcher is worse than no check, because it trains the reader to ignore it.
(2) THE MISSING-KEY RULE WAS DEAD CODE ON THE FORMAT THIS CHANGE INTRODUCED. record() writes surfaces[name] and intervals[name] together, so intervals is always a SUBSET of surfaces, and the check derived its expectations from the payload's own maps - making the not-in-present branch unreachable. Measured: a logs thread that never recorded read ARMED at 100 s, 2000 s AND 100000 s. The three grace-window tests were green against a payload the writer cannot emit. Fixed by taking the expected surfaces from session_plan, with the plan as an interval FLOOR - a heartbeat cannot be the authority on which surfaces should have reported, because the failure mode is a surface that never wrote anything.
(3) THE VERDICT PROSE STATED A FALSEHOOD OVER A 14-MINUTE WINDOW. A real heartbeat always has written >= every surface stamp, so a WHOLE-WATCHER stall of 70 to 900 s landed in SURFACE_STALE, whose reason asserted the process was 'still flushing'. Fixed with a derived all_surfaces_stale property and a fresh_surfaces evidence field, so the prose is derived from evidence rather than from an assumed mechanism. That was the root error: asserting a mechanism nobody checked.
A TEST THAT LOOKED LIKE A WEAKENING WAS NOT. A slice modified a cycle-38 test whose surfaces map inherited the written stamp, so it had silently been asserting that four surfaces frozen 900 s read as ARMED - the exact blindness 4f removes. The refutation reconstructed the old test against the new code out of tree: 3 failed, 85 passed, only that test. The assertion line is byte-identical; only the fixture became explicit. Isolation, not weakening - and the opposite call from cycle 38, where a similar-looking change WAS hiding a hole.
A MEASUREMENT OF MINE WAS WEAK AND THE REFUTATION'S WAS BETTER. I sampled a healthy watcher ONCE per run and got logs 60 s behind at 80 s and 90 s behind at 420 s, then correctly refused to call either the worst case, since the observed age is phase-dependent. The refutation sampled every 10 s across 330 s and took the MAX. One sample of a phase-dependent quantity is an anecdote; sampling and taking the max is the instrument.
I ALSO GOT A BOUND WRONG, IN THE SAFE DIRECTION. I derived a healthy surface's worst age as poll + 2*flush, making the fast-surface margin look like 1.10x and alarmingly tight. A flush fires whenever ANY surface records, and the 3 s surfaces record ten times per window, so the true bound is poll + flush = 33/60/330 s and the margins are 2.1x/2.5x/2.9x. The refutation's sampled run measured savegames at exactly 33.0 s - the bound touched, not approached.
THAT CORRECTION PRODUCED A BETTER ARGUMENT THAN THE ONE I SPECIFIED THE ITEM WITH. The per-surface bound holds only while SOME surface still records. If every surface stops, no flush fires, the combined stamp freezes, and STALE fires first because it is decided earlier in the chain. STALE and SURFACE_STALE cover each other's blind spot; that is the structural reason to keep them separate, not a reporting preference.
CORRECTION TO A CLAIM I MADE MID-SESSION: I said 4f detects the existing DestinationInsideRepoError early return in poll_forever, which stops a surface's stamp. True, but far narrower than I implied - retarget returns early when the day is UNCHANGED, so that exception can only be raised at a MIDNIGHT ROLLOVER, once a day at most.
A MEASURED NEGATIVE, recorded so nobody re-derives it: the 4e-era worry that a source directory the game has not yet created might never produce a pass is FALSE - poll_once catches OSError and stamps anyway.
No backoff constant was invented for the failed-flush retry. A persistent replace-stage failure has never been observed on this machine, and that measurement is what a backoff would have to be derived from. Omit rather than guess.
Pid 23628 still reads NO_HEARTBEAT with armed=True and was neither re-armed nor stopped.

### LL-0122 - 2026-09-03 - ROADMAP 4e CLOSED - the wrap now re-checks the watcher's identity and its heartbeat, and two of the item's own premises were false

**Evidence:**
- Suite 1518 passed in 104.04s, run BARE and read at the wrap; baseline 1430 measured the same way before dispatching. ruff check . clean. merge_gate.verify(baseline=1430) OK at 1509 before the wrap's own additions.
- ops/loop/watch.py: check_watcher() returns one of NO_RECORD, DEAD, IMPOSTOR, NO_HEARTBEAT, STALE, ARMED. ensure_armed_at_wrap() re-arms on the first three only. Nothing is ever terminated - there is still no stop path.
- Identity is the process CREATION TIME via ctypes GetProcessTimes, not the command line, against IDENTITY_TOLERANCE_S = 120.0 s. Measured ground truth: pid 23628 was created 0.056876 s AFTER its recorded started stamp, and _now() truncates microseconds, so the window is deliberately generous - a false IMPOSTOR re-arms beside a live watcher, which is the failure ensure_armed exists to refuse.
- psutil is installed on this machine but is NOT a declared dependency; pyproject.toml keeps dependencies = [] and the probe is stdlib ctypes, following the OpenProcess idiom already in ops/loop/guard.py.
- lanternlight/armwatch.py: --heartbeat PATH writes ops/runtime/armwatch_heartbeat.json atomically, rewritten never appended, first flush immediate then throttled to HEARTBEAT_FLUSH_INTERVAL_S = 30.0 s. Absent flag = byte-identical prior behaviour; --heartbeat with --dest-root REFUSES rather than silently ignoring the flag.
- MEASURED END TO END by the merger against the real CLI, not fixtures: 3 passes x 4 surfaces recorded passes=12 while exactly ONE file was archived, with all four surface stamps present. Control with the flag absent created no file. That is 4e's third failure mode demonstrated - the heartbeat advances when nothing is archived.
- HEARTBEAT_STALE_AFTER_S = 900.0 = 3 x SLOWEST_POLL_INTERVAL_S. Clears the 330 s a healthy watcher can honestly take (LOG_POLL_S 300 + flush throttle 30), which is now PINNED by a test rather than by two comments agreeing.
- Two tests WATCHED GOING RED, verbatim: mutating _identity_matches to return True gave "assert 'NO_HEARTBEAT' == 'IMPOSTOR'" across 3 tests including the acceptance-named live-pid one; mutating the staleness comparison to > float('inf') gave "assert 'ARMED' == 'STALE'". Both reproduced independently by the refutation pass.
- Slice A ran six mutations and TWO SURVIVED its first sweep: naive_clock (every test injected a clock, so the production default never ran) and not_atomic ("no temp file survives" is a negative assertion, equally satisfied by never making one). Both closed, then observed red.
- docs/HEADLESS.md 4b states the wrap-side check beside the entry-side one; .claude/commands/done.md gained step 8 and a matching line in its Definition of done. Both are pinned by SESSION_WRAP_DOCS, the sibling of the 4d entry-doc test.

TWO OF 4e's OWN PREMISES WERE FALSE. Both withdrawals are written ADJACENT to the claims in ROADMAP.md, not only here - cycle 37's rule 2 is that a withdrawal the reader does not meet where they meet the claim does not reach them.
(1) "ensure_armed produced no armwatch.log where a direct invocation does" - there is NO such asymmetry. No code path in this repository writes armwatch.log; a FileHandler|basicConfig|getLogger sweep returns nothing with its positive control passing. The 2026-08-31 file is 562 bytes of run_rolling's startup banner with a 0-byte armwatch.err beside it - a hand-typed shell redirect. The real difference is that default_spawn sends a detached child's streams to DEVNULL deliberately, now documented.
(2) "There is no observation that separates correctly idle from wedged" - one exists and needs no code. Win32_Process.OtherOperationCount for pid 23628 climbed 508/15 s, 971/30 s, 266/10 s with Read, Write and CPU flat and Threads=5, matching what poll_once predicts. Controls: idle Python 0, scan-only Python 1442, four pwsh 0.
MY OWN CLAIM WAS OVER-STATED AND THE REFUTER WAS RIGHT. The counter is per-PROCESS, so it proves the watcher is not WHOLLY wedged, not that every surface is polling. The logs surface is about 0.5 percent of that traffic. I stated the limitation but not strongly enough, and the corrected wording is what stands in ROADMAP 4e.
THE REFUTATION FOUND A HOLE THE MERGER'S OWN REVIEW MISSED. Slice B narrowed a safety guard that blocked its own feature - the textbook conflict of interest - by removing OpenProcess from a forbidden set and adding a check on the RIGHT instead. That was legitimate, but the replacement collector matched ast.Attribute only, so a bare OpenProcess(...) after a rebinding walked through a spelling the old guard caught. I adjudicated that change and passed it. Fixed at the wrap, with a guard FOR the guard over five synthetic evasion spellings and a mirror test; observed red as "the collector never saw 'OpenProcess(PROCESS_ALL_ACCESS, False, pid)'" with exactly 1 of 5 failing, which is what proves the new branch load-bearing.
A green suite is not evidence a guard was not weakened. The suite was green at 1509 with that hole open, and the merge gate said OK.
SPLIT OUT RATHER THAN LEFT IMPLIED: ROADMAP 4f - the verdict rests on the combined written stamp, so ONE wedged surface out of four still reads as ARMED. docs/HEADLESS.md previously claimed the per-surface map prevented exactly that; the code disclaims it, so the doc was false and is corrected. The map is EVIDENCE, not a verdict.
ALSO FILED: OPS-16 - the termination-path guard is blind to taskkill-through-a-string-argument, getattr-assembled attribute names, and ntdll entry points. All three PREDATE this cycle; the refutation replayed both guards over HEAD's module to separate what the change lost from what was never caught.
The anti-cheat boundary is clean: the shipped ops/loop/watch.py calls kernel32 OpenProcess, GetProcessTimes and CloseHandle, plus one Popen with a fixed argv, and asks only for PROCESS_QUERY_LIMITED_INFORMATION (0x1000). No terminate, suspend or memory-write path exists.
Pid 23628 was NOT re-armed and NOT stopped. It runs pre-heartbeat code, so check_watcher reports it NO_HEARTBEAT with armed=True - re-arming it would put a second poller on the same four sources while OPS-14 is open.
...
**Evidence:**
- python -m pytest -> '1423 passed', exit 0, run bare as its own command; 'python -m pytest -q' would have printed no summary line at all
- python -m ruff check . -> 'All checks passed!', exit 0; collect 1423 across 29 test files against a 1416 baseline measured BEFORE dispatching; ops.merge_gate.verify(baseline=1416) -> OK
- read_frame over the 124 panel-up frames: 118 read at the single shipped row y=390, 120 by consensus, 0 lost, 0 misread against the human transcription, 0 disagreements across the five rows
- the two recovered frames are EXACTLY f0469 and f0470, the pair ROADMAP 7c named
- 0 of 231 panel-down frames read; 0 of ALL 1,817 out-of-window frames read - the full population, not the 120-frame sample the previous pass checked
- an independent chrome-mask NCC detector that never calls read_panel: panel-up floor +0.5274, panel-down ceiling +0.3010, out-of-window max +0.3588, ZERO out-of-window frames reach the floor; validated at Pearson r = 0.945 against the transcription's own header_ncc column. So 124 panel-up frames is a TOTAL, not a floor
- 6,439-frame consumer diff, HEAD 9879c60 against this tree: 3,243 identical, 3,196 both refused, 0 GAINED, 0 LOST, 0 CHANGED, and 0 differences in refusal MESSAGE text
- that diff harness was proved non-vacuous by mutation before its zero was believed: ACCEPT_DISTANCE 0.115->0.010 gave 238 LOST, a shifted HITS_WINDOW gave 87 LOST and 2 CHANGED, 0.115->0.400 gave 2 GAINED
- all 14 pre-existing module functions compile to identical co_code AND co_consts; read_panel's source bytes are unchanged, by LF-normalised comparison
- read_frame over all 6,439 pre-cropped 500x310 panels: 6,439 refuse 'too small', 0 do anything else, with a positive control - a synthesised full scene with a panel pasted at the measured origin reads 99/8
- six mutations, anchor asserted to occur exactly once before each, module restored byte-exact by sha256 after every one: pick-a-winner 1 RED, collapse to (390,) 2 RED, drop the size precondition 1 RED, drop the no-reading refusal 1 RED, shrink the band to (389,390,391) 1 RED, add an unsafe row 386 1 RED
- consensus DEPTH over the 124: 2 frames read at all five rows, 37 at three, 25 at two, 56 at exactly ONE row, 4 at none; and 0 misreads at every one of the five rows measured individually
- x sweep at y=390 over the 124: x 2057-2065 each read 118 with zero wrong, x 2056 reads only 110, and x 2048, 2066 and 2068 return 30 WRONG readings between them
- hits field geometry over the 124: LEFT-aligned (left extent 199-200 at both 1 and 2 digits), rightmost lit column 223 against a last usable column of 223 - ZERO right margin
- truncation demonstrated on real ink: f0539_00.42.52.png, true live_hits 14, read through window (193, 212) returns 1
- f0661 is a FADE not a misalignment: brightest value-window pixel rgb(106, 65, 24) against rgb(234, 128, 21) in both neighbouring frames, and 0 lit pixels in its whole panel crop

THE REGISTRATION SEARCH IS DONE WITHOUT BEING A SEARCH. read_frame crops a full frame at every row in FRAME_CONSENSUS_ROWS (388-392, x fixed at 2058) and requires the readings to AGREE; a conflict REFUSES. It never picks a winner, so it cannot hunt for an alignment that makes a glyph match, and two rows returning different numbers is a refusal trigger that did not exist before. read_panel is untouched and the 6,439-frame reference surface is unmoved.

THE ITEM'S OWN PREMISE WAS FALSE, AND THIS SESSION REPEATED IT UNMEASURED. ROADMAP 7c says f0469 and f0470 'sit 2px high because the panel is still sliding in'. Measured: both read at y=389 and REFUSE at 388 and 392, so the effective offset is 1px. It is not a translation at all - their value ink band is rows (2, 21), height 20, against (5, 23), height 19, on a settled frame: 3px at the top, 2px at the bottom, one row TALLER. The panel is SCALING in, not sliding. The hits band is (3, 20) in both, unshifted, which is why a 2px row refuses on hits rather than on value. The claim was copied into two new docstrings before the refutation caught it - THE FIFTH CONSECUTIVE CYCLE with a false claim in the file that documents the reader, and the first where the false claim was INHERITED rather than invented.

THE REBUTTAL IN THE DOCSTRING WAS AIMED AT THE WRONG OBJECTION, and the caveat is now written down. ROADMAP 7c declined a search because it 'multiplies scoring attempts and so erodes the margin guarantee'. read_frame multiplies them by five exactly as a search would; what it does not do is CHOOSE among the results. That is a narrower rebuttal than the objection deserves, and it matters most where the cross-check is thinnest: 56 of the 120 frames are read by exactly ONE row, including both frames this function exists to recover, and for those there is no cross-check at all. What was measured is that no individual row misread any frame. A CAVEAT STATED IN CHAT AND DROPPED FROM THE ARTIFACT IS A LIE IN THE ARTIFACT, so it is in the read_frame docstring now.

THE BAND WAS PINNED BY NOTHING AND NOW IS PINNED FROM BOTH SIDES. Shrinking FRAME_CONSENSUS_ROWS to (389, 390, 391) left the entire suite green at 1422 - the two outer rows bought crop tolerance that no test defended. The new test asserts the exact band, that every row reads something, and that NO row ever misreads; shrinking it now goes RED and so does adding an unsafe row 386. This is LL-0116's 'pinned from both sides' shape applied to a constant on the way IN rather than a cycle later.

read_frame WAS MISSING FROM __all__, so a star import silently exported read_panel and not the new function. Found by the refutation EXECUTING the import rather than reading the list. Fixed.

A LATENT WRONG-NUMBER DEFECT FOUND WHILE MEASURING SOMETHING ELSE, recorded as ROADMAP 7d rather than hot-fixed. _read_field never asserts that ink stops before x_hi, so a glyph pushed entirely outside a field window is silently dropped and the survivors form a valid number. Demonstrated on real captured ink: a frame whose true hits is 14 reads as 1 through a window that cuts the last digit. A PARTIAL cut fails safe - the fragment lands in the ambiguity band and refuses - and a CLEAN cut fails dangerous, which is why it has no detector. It is live: both fields are LEFT-aligned, and the hits window has ZERO right margin at 2 digits, so hits reaching 100 would read as 10. It has never fired because the capture's maximum live_hits is 50. NOT hot-fixed, because widening the window means choosing a bound without a capture that contains a 3-digit hit count, which is LL-0116's exact failure - and because the tidy guard 'refuse if ink touches the last column' would refuse 78 measured frames. A lookahead guard is proposed instead, with the measurement that makes it safe recorded beside it: the 12 columns outside each window, on each side, are empty in all 124 frames.

THE MODULE DOCSTRING'S x CLAIM WAS FALSE AT BOTH ENDS and is corrected. It advertised the x origin as 'tolerant across 2056-2061'. Measured: 2056 costs 8 readings, 2062-2065 are as good as 2058 and were excluded, and horizontal misalignment DOES produce wrong numbers - 30 of them across x 2048, 2066 and 2068 - where the docstring's 'never produces a wrong one' is true only of the VERTICAL axis. The danger is not monotonic: 2048 gives 12 wrong while 2050 gives none. x is therefore fixed at 2058 and never swept, and a 2-D sweep was measured to recover ZERO extra frames over the y sweep alone.

FOUR INSTRUMENTS REPORTED FALSELY THIS SESSION AND EACH WAS CAUGHT ONLY BY A SECOND METHOD. 'grep -c' reported 0 carriage returns in a file from which tr removed 602. 'cmp' reported a content difference at byte 68 that was purely CRLF against LF - which read as a false alarm here, and with matching sizes could equally have read as a false green. A co_consts comparison reported five functions changed when nothing had, because nested code objects compare by identity across two separately loaded modules. And a hand-rolled PII positive control silently wrote its planted SteamID64 to C:/Program Files/Git/ because Git Bash maps a leading slash to the Git root, so the control tested nothing; the guard itself is sound and carries its own planted-file controls, which passed. AN EMPTY OR PASSING RESULT IS A CLAIM ABOUT THE INSTRUMENT.

LL-0117'S OWN BYTE EVIDENCE DOES NOT RECONCILE. It states vision_meter.py is '26,734 bytes with 602 CRLF pairs on disk and 25,879 bytes with none as a git blob'. Measured: HEAD 9879c60 is 602 lines and 26,132 bytes as a blob, so the working figure 26,734 and the 602 pairs are both correct for HEAD - but 25,879 is the blob at 98d2cac, the PREVIOUS commit, which is 592 lines. The bullet's own arithmetic fails: 26,734 minus 602 is 26,132, not 25,879. This is the intermediate-tree measurement error again, landing inside the very bullet written to warn that a size quoted as evidence must name which form it is.

TWO GROUND-TRUTH QUESTIONS RECORDED, because they belong nowhere else. The transcription's header_ncc column has NO producer anywhere in the repository - confirmed by an independent case-insensitive sweep of every .py and .md - so a fresh clone cannot regenerate or check the column a detector was validated against. And f0661 is labelled panel_state 'up' with live_total 9 while its panel crop contains zero orange pixels; the label and the pixels disagree because the frame is mid-fade, and a human can read a dimmed glyph that a colour threshold cannot. Neither blocks this item.

ITEMS 7, 11 AND 12 REMAIN OPEN AND UNCREDITED. The client was closed for the whole session, confirmed at entry and again mid-session by filtering on the process NAME with a positive control. The session watcher refused to re-arm because pid 23628 was already running, and unlike LL-0117 that refusal was VERIFIED rather than trusted: the pid was alive, its command line named lanternlight.armwatch, and its start time matched the arming record. It was still alive at the wrap. ROADMAP 4e was opened for the wrap-side check that LL-0117 asked for.

### LL-0117 - 2026-09-01 - WRAP - the session watcher was found DEAD at the wrap and re-armed, and a withdrawn ROADMAP claim was still asserted in the present tense below its own withdrawal

**Evidence:**
- python -m pytest -> '1416 passed', exit 0, run as its own command at the wrap
- python -m ruff check . -> 'All checks passed!', exit 0; collect 1416 across 29 files
- armwatch.json recorded pid 17568, which was ALIVE at session start (ensure_armed refused twice on it) and DEAD at the wrap
- ensure_armed re-armed as pid 23628 into C:/ll-captures/2026-09-01, and the new pid was verified alive after arming rather than assumed
- loop lock released: owner pid 69188 (the loop sentinel) had died, the guard already reported the lock as not held, the lock file was removed and its recorded owner is now None
- case-insensitive whitespace-collapsed sweep over every tracked .md and .py for 9 stale patterns -> one real hit, ROADMAP's kept sub-item body
- 'python -m pytest -q' prints NO summary line and exits 0, because pytest.ini already carries -q so a second makes it -qq; 'python -m pytest' bare prints '1416 passed'
- lanternlight/vision_meter.py is 26,734 bytes with 602 CRLF pairs on disk and 25,879 bytes with zero as a git blob, against a .gitattributes that pins *.py to eol=lf
- the 354 misfire / 85 comma split of LL-0116 reproduces ONLY under the criterion 'four-digit judged by the transcription'; under the criterion LL-0115 published for itself, 'requiring the frame to actually read', it is 355 and 84 - the single differing record is f0581, a genuine comma in a frame that does not read

THE SESSION WATCHER DIED MID-SESSION AND NOTHING NOTICED. It was alive at 07:50:14 as pid 17568 and refused two arming attempts during the session, which is the correct refusal - but by the wrap it was gone, so for an unmeasured stretch nothing was archiving the log, the saves or the market cache. This is the exact gap items 4c and 4d exist to close, and it failed in a new way: not 'nobody armed it' but 'it was armed, it refused a re-arm because it was alive, and then it died'. A REFUSAL TO RE-ARM IS ONLY AS GOOD AS THE PROCESS IT DEFERRED TO, and nothing re-checks liveness between the arming and the next session entry. Re-armed as pid 23628 and verified alive.
THE ARMING CHECK BELONGS AT THE WRAP AS WELL AS AT THE START, which no session-entry document currently says. Every entry path calls ensure_armed on the way IN; none checks on the way OUT, and the way out is precisely when a session hands the machine back to an operator who is about to launch the client. Recorded here rather than fixed, because changing the documented start-up contract is a ROADMAP item and not a wrap edit.
A WITHDRAWN CLAIM WAS STILL ASSERTED BELOW ITS OWN WITHDRAWAL. ROADMAP's sub-item 2 was struck through and corrected in its heading, and the kept original body four lines below still read, in the present tense, that the predicate accepts any sub-6px run with first row >= 12 and height <= 10 and that none ever reached a number. Both are now false - the predicate also requires height >= 4, and 30 of those firings are a genuine comma. This is LL-0109's shape exactly, a withdrawn claim restated below its own withdrawal, and it was found by the merger's own sweep rather than by a refuter.
TWO INSTRUMENTS THAT REPORT SUCCESS WHILE MEASURING NOTHING, both found by the wrap refutation and both re-measured here. 'python -m pytest -q' emits dots, a percentage and NO SUMMARY LINE, and exits 0 - pytest.ini already carries -q, so a second one makes it -qq. Every count quoted from a -q run this session would have been read from a line that was never printed; the counts here were read from a bare run, which is why they survived. And a sha256 of a working .py file is NOT the sha256 of the commit: .gitattributes pins *.py to eol=lf while the working tree is CRLF, a 855-byte difference on this one module, so a hash quoted as evidence must name which form it is and only the git blob is reproducible from a clone. Both are now in CLAUDE.md, where they are loaded every turn, rather than only here.
LL-0116's OWN CLAIM THAT 'THE CRITERION IS PUBLISHED BESIDE EACH NUMBER' IS FALSE FOR ONE PAIR, found by the wrap refutation. Its 354 misfires and 85 genuine commas reproduce only under 'four-digit judged by the transcription'; under the criterion LL-0115 published for itself - 'requiring the frame to actually read' - the same measurement gives 355 and 84, the one differing record being f0581, a genuine comma in a frame that does not read. Neither the ledger bullet nor the module nor the test said which was used. The criterion is now stated in this entry's evidence. WRITING 'THE CRITERION IS PUBLISHED' IS NOT PUBLISHING IT.
THE SWEEP THAT FOUND IT WAS CASE-INSENSITIVE AND WHITESPACE-COLLAPSED, and an earlier sweep this session missed a defect for want of both. Every other hit across the tree was legitimate withdrawal text quoting the claim it retracts, which is the intended pattern and must not be tidied away.
NO CYCLE-37 WORK WAS DONE. The loop state points at 7c-consensus-registration as the in-flight item with its acceptance criterion recorded and its measurement already taken in scratch. Nothing is credited for it.
ITEMS 7, 11 AND 12 REMAIN OPEN AND UNCREDITED. The client was closed for the whole session, confirmed by filtering on the process NAME. Nothing was stopped that this session did not start: the loop sentinel was its own and the watcher was re-armed rather than replaced.

### LL-0116 - 2026-09-01 - CORRECTS LL-0115 - the separator height floor was justified by a sweep that omitted the one offset where it refuses 45 real commas, and three more of that entry's claims do not survive

**Evidence:**
- python -m pytest -> '1416 passed', exit 0, run as its own command after the corrections
- python -m ruff check . -> 'All checks passed!', exit 0; collect 1416 across 29 files
- genuine comma height BY CROP OFFSET over all 55 four-digit frames, y=388 INCLUDED this time: y388 {4:45, 5:10}; y389 {6:54, 7:1}; y390 {6:1, 7:53, 8:1}; y391 {8:54, 9:1}; y392 {9:54}
- minimum genuine comma height across the advertised 388-392 band: 4, not 6
- floor trade re-derived: floor 3 removes 52 of 354 misfires and loses 0 commas; floor 4 removes 342 and loses 0; floor 5 removes 348 and LOSES 45; floor 6 removes 348 and loses all 55
- misfire height histogram re-derived: {1:25, 2:27, 3:290, 4:6, 6:2, 7:2, 8:2} = 354 - it does NOT stop at 4, and six misfires sit inside the genuine comma's own range
- the floor is now pinned to exactly 4: values 2 and 3 fail the speck test, 5 and 6 fail the shortest-offset test, only 4 is green
- blast radius of the 5 -> 4 correction: crop sweep y=388..392 reads 2/55/118/50/2 with ZERO disagreements at every offset, 0 of 231 panel-down frames read, and the 6,439-frame consumer diff against the pushed tree is 3,243 identical / 3,196 both refused / 0 gained / 0 LOST / 0 CHANGED
- a mutation loop timed out mid-iteration and left the module at SEPARATOR_MIN_HEIGHT = 7; the restore check caught it by sha256 and cmp, and the module is byte-identical to its pre-mutation copy

THE FLOOR SHIPPED AT 5 AND IS CORRECTED TO 4, because the table justifying it OMITTED y=388. That table swept 389, 390, 391 and 392 and stated 'a genuine comma that is never shorter than 6' - inside a paragraph invoking 'every offset in 388-392' as the justification. At y=388 the comma is height 4 in 45 of the 55 four-digit frames, so the floor of 5 refused 45 real commas at an offset the module advertises as tolerated, and 6 would have refused all 55. THE CYCLE THAT CONGRATULATED ITSELF FOR NOT TIGHTENING SEPARATOR_MIN_ROW COMMITTED THE IDENTICAL ERROR ON THE CONSTANT IT ADDED, one paragraph away - which is LL-0111's shape and LL-0113's shape arriving a third time.
THE OMISSION WAS IN THE PROBE, NOT IN THE WRITE-UP. The merger's own sweep looped over (389, 390, 391, 392); the module's advertised band is 388-392. Every downstream artifact - the module docstring, the test docstring, the ledger evidence bullet and the commit message - faithfully reported a table that was already missing its worst case. A DOCUMENT CANNOT BE MORE COMPLETE THAN THE PROBE BEHIND IT, so when a constant is justified by a sweep, the sweep's range must be read off the property being defended rather than typed from memory.
MARGIN WAS THE WRONG FRAME AND IT REASONED THE FIRST VERSION INTO A WORSE ANSWER. The rejected justification was '5 rather than 6, because a floor at the observed minimum would have zero margin'. Both directions of this constant fail SAFE - too high refuses a real comma, too low admits a misfire that _regroup then rejects - so the rule is not 'leave margin', it is NEVER REFUSE MEASURED DATA. 4 is the largest floor that refuses no genuine comma at any offset in the band, and it still removes 342 of the 354 misfires.
'THE CONSTANT IS PINNED FROM BOTH SIDES' WAS FALSE WHEN WRITTEN and is true now. The refutation demonstrated the old floor drifting silently to 4 or to 6 with the whole suite green. With the shortest-offset test added, 2 and 3 fail the speck test, 5 and 6 fail the shortest-offset test, and only 4 passes.
THREE FURTHER CLAIMS OF LL-0115 DO NOT SURVIVE, all documentation rather than behaviour. 'All nine over-wide pieces were caught by the DISTANCE threshold alone' is false - three never reach the classifier at all and a fourth refuses at the reject bound, so it was six; the same test file said six correctly forty lines below its own contradiction, which is LL-0114's self-contradiction defect repeating one commit later. The misfire histogram was published as heights 1-4 summing to 348 when the measured population is 354 and includes six runs at heights 6, 7 and 8 INSIDE the comma's range, so the clean separation the prose implied does not exist. And ROADMAP's kept sub-items still asserted in the present tense that no misfire ever reached a number, which LL-0115's own body refutes.
WHAT SURVIVED THE REFUTATION UNBROKEN, independently re-derived by the refuting pass against the pushed commit: 1415 passed with zero skips at that tree, the collect baseline of 1411 with only test_vision_meter.py changing, 118 read and 6 refused with zero disagreements, the five-offset sweep, zero panel-down readings, the 6,439-frame diff at 0 changed and 0 lost and 0 gained, the four filed mutations each going red in the named place, the 9 over-wide pieces and the 6-versus-3 split, the 3 _regroup-only frames all with grouping [0, 0], and the anti-cheat and PII sweeps with positive controls. THE CODE WAS SAFE AND THE DOCUMENTATION WAS NOT.
A KNOWN PRE-EXISTING WEAKNESS, RECORDED NOT FIXED. The refutation synthesised a 3px-wide 5px-tall orange speck beside a real value and made one frame read 4060 for a true 1060. It reproduces byte-identically against the tree before this cycle, so it is not a regression: it comes from the gap=2 run merge and is the same family as the merged-run problem. It requires injecting pixels into the capture, which is not a threat model this module has - but it is the honest limit of 'no input in either capture yields a wrong number', which is a claim about THESE captures and not about all possible frames.
A COUNTING DIFFERENCE RECONCILED RATHER THAN ADJUDICATED, for the third cycle running. The refutation measured 18 frames whose refusal MESSAGE changed where this entry published 10 whose refusal GUARD changed; eight more frames changed which run is named inside the same fragment guard. Both are exact. It also measured 84 genuine commas and 355 misfires against the merger's 85 and 354, the single differing record being f0581 - a genuine comma in a frame that does not read, which the merger's own stated criterion should have excluded. The criterion is published beside each number.
A MUTATION LOOP TIMED OUT AND LEFT THE MODULE MUTATED. The restore step never ran for that iteration, and SEPARATOR_MIN_HEIGHT sat at 7 in the working tree. It was caught because the restore is VERIFIED by hash and cmp rather than assumed to have happened. A CLEANUP STEP THAT ONLY RUNS ON THE SUCCESS PATH IS NOT A CLEANUP STEP, and had this been followed straight by a commit it would have shipped a constant nothing chose.
ITEMS 7, 11 AND 12 REMAIN OPEN AND UNCREDITED. The client was closed throughout, confirmed by filtering on the process NAME.
