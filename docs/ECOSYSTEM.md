...
| `docs.github.com` | GitHub's own product documentation | **Repository governance only, never evidence about this game.** Cited by `CODE_OF_CONDUCT.md` and `SECURITY.md` for GitHub's abuse-reporting and private-vulnerability-reporting instructions. A separate host from `github.com` above - a parent and its subdomain are different operators and this register keeps them on different rows |
| `contributor-covenant.org` | Publisher of the Contributor Covenant, the community code-of-conduct text this project's `CODE_OF_CONDUCT.md` borrows the shape of | **Repository governance only, never evidence about this game.** Cited once, as attribution for that borrowed shape. Not assessed for anything else and there is nothing else to assess it for |
| `x.com`, `t.co` | Named in `ROADMAP.md` (`OPS-57`) ONLY as the two-label fragments that hide inside `gamingpromax.com` and `grindnstrat.com`, which is the worked example for why `_present_in_register` requires a host boundary instead of a substring | **NOT SOURCES - registered, not vetted, and cited as fragments rather than as sites.** Nobody here has fetched either. They are listed because they are real hosts and a real host must never be hidden in a denylist: `tests/test_source_register.py` keeps non-hosts in `KNOWN_NON_HOSTS` and sends every host here, so a row saying "not a source" is the honest home for one. Assess it before citing it for anything |
